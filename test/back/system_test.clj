...
(deftest config-init-test
  (testing "initialisation of the config map"
    (is (= {:port nil :open-browser? nil :browse-url "http://localhost:null/"}
           (sys/init-app-config  {})))


    (is (= {:port 111 :open-browser? nil :browse-url  "http://localhost:111/"}
           (sys/init-app-config  {:port 111})))

    (is (= {:port 222 :open-browser? nil :browse-url "http://localhost:222/"}
           (sys/init-app-config  {:port 111
                                  :user-config {:user-config/server-port 222}}))
        "user configured port value takes priority over default")

    (is (= {:port 222 :open-browser? true :browse-url  "http://localhost:222/"}
           (sys/init-app-config  {:port 111
                                  :user-config {:user-config/server-port 222
                                                :user-config/open-browser true}}))
        "open-browser read from user config")

    (is (= {:port 222 :open-browser? false :browse-url  "http://localhost:222/"}
           (sys/init-app-config  {:port 111
                                  :open-browser? true
                                  :user-config {:user-config/server-port 222
